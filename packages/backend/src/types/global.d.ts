/**
 * Global type augmentations for the Syra backend.
 *
 ...
 * the Express `req`/`app` (e.g. the live-rooms routes) can reach it. Declaring
 * it here gives every consumer a properly typed `global.io` (and
 * `globalThis.io`) instead of an `as any` cast.
 *
