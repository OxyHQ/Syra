...
/**
 * Base URL of the Syra catalog API. Optional env override; when unset the SDK
 * falls back to its built-in Syra API default. Trailing slashes are trimmed so
 * the SDK builds clean request paths.
 */
const SYRA_API_URL = process.env.SYRA_API_URL?.replace(/\/+$/, '');

/**
 * The single shared Syra catalog client. Every backend path that resolves Syra
 * podcasts (the live-room podcast queue) reuses this one instance instead of
 * constructing its own.
 */
export const syraClient = createSyraClient(SYRA_API_URL ? { baseURL: SYRA_API_URL } : {});
