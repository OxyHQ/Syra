...
export const Search = ({ color: colorProp, size = 26, style }: { color?: string; size?: number; style?: ViewStyle }) => {
...
export const SearchActive = ({ color: colorProp, size = 26, style }: { color?: string; size?: number; style?: ViewStyle }) => {
  const theme = useTheme();
  const color = colorProp ?? theme.colors.icon;
  return (
    <Svg viewBox="0 0 24 24" width={size} height={size} style={{ ...style }}>
